...
from ..observations import ObservationSequence, State
...
class SensorModel(object):
    ...
    def observe_state(self, state):
        ...
        observation = []
        for k in self.s_to_o.keys():
            if k.issubset(state_literals):
                values = list(self.s_to_o[k].keys())
                probs = list(self.s_to_o[k].values())

                if sum(probs) != 0:
                    observed_value = np.random.choice(values, 1, p=probs)[0]
    ...
    def observe_trajectory(self, trajectory):
        obs_sequence = dict()
        for order,state in trajectory.states.items():
            obs_sequence[order] =  self.observe_state(state)

        return ObservationSequence(obs_sequence, all_states_observed=True, all_actions_observed=False)


    def decode_observation(self, observation):
        landmarks = []
        for k in self.o_to_s.keys():
            if k in observation.literals:
                landmarks += list(self.o_to_s[k].keys())
...
class Plan(object):
    def __init__(self, actions, cost = -1):
