...
def get_matching_literals(F, predicates, objects, restricted=False):
    matching_literals = []
    ...
    for l in [f for f in F if f.predicate in p_names]:
        if len(l.args) == 0:
            continue
        elif not restricted and len(o_names.intersection(set(l.args))) != 0:
            matching_literals.append(l)
        elif restricted and len(o_names.intersection(set(l.args))) == len(l.args):
            matching_literals.append(l)
